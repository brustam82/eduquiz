...
// ── пароль: храним не текст, а «отпечаток» ──
...
// ── регистрация: только из Telegram ──
async function register(req, env) {
  const b = await req.json();
  const u = await checkInitData(env, b.initData);
  if (!u || !u.id) return J({ error: 'bad_initdata' }, 401);

  const login = String(b.login || '').trim();
  const name = String(b.name || '').trim().slice(0, 80);
  const pass = String(b.password || '');
  if (!/^[a-zA-Z0-9_.]{3,20}$/.test(login)) return J({ error: 'bad_login' }, 400);
  if (name.length < 2 || pass.length < 6) return J({ error: 'bad_input' }, 400);

  const tgid = String(u.id);
  const busy = await sb(env,
    `teachers?or=(login.eq.${encodeURIComponent(login)},tg_chat_id.eq.${tgid})&select=login,tg_chat_id`);
  if (busy.some(x => x.login === login)) return J({ error: 'login_taken' }, 409);
  if (busy.length) return J({ error: 'tg_taken' }, 409);
  ...
  const saved = await sb(env, 'teachers', {
    ...
    headers: { Prefer: 'return=representation' },
    body: JSON.stringify({
      login, name, tg_chat_id: tgid,
      pass_hash: await pwHash(pass, salt),
      pass_salt: b64(salt)
    })
  });
  const me = saved[0];
  return J({ ok: true, token: await mkToken(env, me.id), me: { name: me.name, login: me.login } });
}

// ── вход по логину и паролю (работает и вне Telegram) ──
async function login(req, env) {
  const b = await req.json();
  const l = String(b.login || '').trim();
  const rows = await sb(env,
    `teachers?login=eq.${encodeURIComponent(l)}&select=id,name,login,pass_hash,pass_salt&limit=1`);
  if (!rows.length) return J({ error: 'auth' }, 401);
  const t = rows[0];
  const h = await pwHash(String(b.password || ''), b64d(t.pass_salt));
  if (!same(h, t.pass_hash)) return J({ error: 'auth' }, 401);
  return J({ ok: true, token: await mkToken(env, t.id), me: { name: t.name, login: t.login } });
}
...
async function me(req, env) {
  ...
  const rows = await sb(env, `teachers?id=eq.${id}&select=name,login&limit=1`);
  if (!rows.length) return J({ error: 'auth' }, 401);
  return J({ ok: true, me: rows[0] });
}
...
export default {
  async fetch(req, env, ctx) {
    ...
    try {
      ...
      if (m === 'POST' && p === '/api/register') return await register(req, env);
      if (m === 'POST' && p === '/api/login') return await login(req, env);
