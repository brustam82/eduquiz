...
async function stats(req, env) {
  ...
  const results = await sb(env,
    'results?select=student,exam_code,total,correct,percent,grade_mark,duration_s,created_at'
    + `&order=created_at.desc&limit=${n}`);
  // название экзамена лежит в другой таблице — подставляем вручную
  const exams = await sb(env, 'exams?select=code,title&limit=200');
  const byCode = {};
